...
{id:"grand",name:"메가시티 (오픈월드)",icon:"🌆",desc:"2.6km² 통합 오픈월드 — 도심·순환고속도로·산악 와인딩·호수·공업·공항·해안도로·스타디움. 📍장소 선택 스폰",
 ...
 build(){
  ...
  // ⛲🅿️ 도심 중앙 광장: 분수 + 대형 주차장(사용자 요청 — 다운타운 중심). 넓게 개방.
  {const fx=0,fz=0;
   mb.stamp(fx,fz,88,(i,j,d)=>w.setS(i,j,d<40?S_WLK:S_ASP));   // 안쪽 보행광장 + 바깥 주차 아스팔트
   mb.texCircle(fx,fz,86,SURF_CSS[S_ASP]);
   mb.texCircle(fx,fz,40,SURF_CSS[S_WLK]);
  ...
  // 🌉 도심 플라이오버(위로 솟았다 내려오는 고가): 동서 오버패스 — 격자 도로 위(건물 미접촉)
  {const z0=90,fo=[];for(let k=0;k<=16;k++){const t=k/16,x=-185+t*370;
     const y=w.height(x,z0)*(1-Math.sin(t*Math.PI))+Math.sin(t*Math.PI)*7.5;
     fo.push({x,z:z0,y});}
   bridgeDeck(mb,fo,14,{deck:0x5a616c,rail:0xb9c2cc});
   mb.texText(0,z0-11,5,"OVERPASS",'rgba(240,244,250,.45)');}
  // 🚧 시작점(스폰 0,-60) 근처 오르내림·요철·방지턱
  w.addRippleZone(0,-90,40,.014,2.2,3);      // 스폰 앞 완만한 꿀렁임
  w.addRippleZone(0,-150,34,.012,2.6,1);     // 스폰 뒤 잔요철
  for(const[bx2,bz2,h,ty]of[[0,-40,.13,"round"],[0,-118,.16,"arch"],[-90,-60,.12,"flat"],[90,-60,.14,"sharp"]])
    mb.bump(bx2,bz2,0,13,h,ty);
  // 베이크 건물 — 중앙 광장(r110)·플라이오버 라인 비우고, 종류·높이 다양하게(더 큰 도심)
  let seed=11;const rnd=()=>{seed=(seed*16807)%2147483647;return seed/2147483647;};
  const BLD2=["bldA","bldB","bldC","bldD"];
  for(let bx=-3;bx<=2;bx++)for(let bz=-3;bz<=2;bz++){
    const cx=bx*90+45,cz=bz*90+45;
    if(Math.hypot(cx,cz)<110)continue;                 // 중앙 광장 비움(개방)
    if(Math.abs(cz-90)<24)continue;                    // 플라이오버 라인 비움
    const dense=Math.hypot(cx,cz)<250;                 // 중심부는 고층 밀집, 외곽은 저층
