...
const CARS=[
 ...
 {id:"maybach",name:"메르세데스-마이바흐 GLS",icon:"🚘",drive:"4WD",mass:2560,hp:621,acc:"4.9초",top:240,
  ...
  /* 요청: 앞바퀴는 뒤로, 뒷바퀴는 앞으로 — 휠베이스를 18cm 좁혀 아치 안으로 당긴다 */
  wheelFrontAdj:-.09, wheelRearAdj:-.09,
  ...
  /* 요청: 더 쫀득하고 SUV답게 — 스트로크는 길게(SUV), 감쇠는 두툼하게.
     한 번 눌렸다가 천천히 되돌아오는 '쫀득한' 거동을 위해 c와 리바운드를 함께 올린다. */
  susp:{k:66000,c:9800,travel:.42,rest:.30,
        prog:1.9,sky:30000,
        compMul:.34,rebMul:3.2,riseMul:3.0,
        fCap:.58,heave:26000,
        preview:.38,pvGain:9,attq:10},
  arb:16000,
 ...
 {id:"veloce",name:"포르쉐 911 터보",icon:"🏁",drive:"4WD",mass:1595,hp:520,acc:"3.2초",top:315,
  ...
  brakeF:9200,steerLo:.56,steerHi:.11,aero:{cd:.5,df:55},
  /* 접지 강화 — 쉽게 미끄러지지 않게. 리어엔진 4WD라 뒤가 더 물린다. */
  gripF:1.30,gripR:1.38,
...
class CarVisual{
  ...
  applyImpact(imp,veh){
    ...
    /* ── 충돌 반동 ──
       범퍼는 무게중심보다 낮다. 그 높이에서 수평으로 받아 급감속하면
       실제로 차가 뒤로 튕기면서 반대쪽(뒤)이 들린다.
       접촉 임펄스만으로는 서스펜션 자세 제어(attq·sky)가 곧바로 눌러 버려
       거의 보이지 않으므로, 반동을 한 번 더 명시적으로 넣고
       잠깐(0.5초) 자세 제어를 풀어 그 움직임이 보이게 한다. */
    if(dv>4&&veh&&veh.body){
      const b=veh.body, sev=Math.min(1,(dv-4)/26);
      // 타격면 반대 방향으로 되튐(월드 좌표)
      _vA.copy(imp.ln).applyQuaternion(b.quat).multiplyScalar(-1);
      const back=sev*(1.6+2.4*sev);                 // 되튐 속도(m/s)
      b.vel.addScaledVector(_vA,back);
      /* 타격점이 무게중심보다 아래면 반대쪽이 들리는 피치 모멘트가 생긴다.
         (임펄스를 접촉점에 걸면 팔길이가 자동으로 그 부호를 만든다) */
      _vB.copy(imp.lp).applyQuaternion(b.quat);      // CoM→타격점(월드)
      _vC.copy(_vA).multiplyScalar(b.mass*back*.16);
      b.applyImpulse(_vC,_vB);
      b.vel.y+=sev*.28;                              // 전체적으로 살짝 들썩
      /* 각속도 상한 — 가벼운 스포츠카는 이 임펄스로 44°까지 곤두섰다(계측).
         '약간 뜨면서 뒤로 밀리는' 정도로만 남긴다. */
      {const w2=b.angVel.length(), LIM=1.15;
       if(w2>LIM)b.angVel.multiplyScalar(LIM/w2);}
      veh.crashKick=Math.max(veh.crashKick||0,.42);}
