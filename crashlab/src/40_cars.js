...
const CARS=[
 ...
 {id:"rrghost",name:"롤스로이스 고스트",icon:"🏛️",drive:"4WD",mass:2490,hp:571,acc:"4.8초",top:250,
  ...
  susp:{k:36000,c:6600,travel:.50,rest:.28,
        prog:1.5,        // 프로그레시브 레이트(바닥칠 직전만 단단)
        sky:15000,       // 스카이훅(차체 상하 요동 직접 감쇠)
        compMul:.15,     // 압축(흡수) 국면은 아주 부드럽게 — 충격이 차체로 안 올라간다
        rebMul:6.0,      // 신장(올라가는) 국면은 쪼인다 — 방지턱 후 차체·뒷축이 솟지 않게
        riseMul:4.5,     // 차체가 상승할 때 헤이브 댐퍼 추가 강화
        fCap:.50,        // 블로우오프 — 서스가 차체를 밀어올릴 수 있는 힘 상한(중력 배수)
        heave:44000,     // 상승 억제: 차체가 위로 뜨려 할 때만 스프링력을 깎는다
        preview:.60,     // 노면 예측 시간(s) — 턱을 미리 읽고 스트로크를 준비한다
        pvGain:20,       // 예측 보정 이득
        attq:20},        // 피치·롤 각속도 감쇠 — 평지와 턱의 거동 차이를 거의 없앤다
  arb:20000,
 ...
 {id:"maybach",name:"메르세데스-마이바흐 GLS",icon:"🚘",drive:"4WD",mass:2560,hp:621,acc:"4.9초",top:240,
  ...
  wheelRadMul:1.49, wheelOutset:.048,
