...
// 부팅 완료까지 폴링(썸네일 9종 + 대형 맵으로 SW-GL에선 ~3s 소요; 실기기 <1s)
let _boot=false;for(let i=0;i<30;i++){await page.waitForTimeout(300);
  if(await page.evaluate(()=>$("loading").classList.contains("off"))){_boot=true;break;}}
