...
## v1.4 (2026-07-05)
- **노드-빔 소프트바디 충돌** (BeamNG식): 차체를 5×4×9 격자(180노드,
  이웃+대각 빔 ~1,400개)로 감싸고 PBD로 시뮬레이션. 빔 변형률 4.5% 초과 시
  소성(영구) 변형 — 충격이 격자를 타고 전파되며 실제처럼 접히고 구겨진다.
  메시 버텍스는 트라이리니어 가중치로 격자를 따라감. 충격 후 ~1초 활성(성능).
- **완전 3D 메뉴**: 메뉴 배경이 라이브 쇼룸(노을 배경·글로우 링 플랫폼) —
  선택한 차가 실물로 회전하고 색상 변경이 실시간 반영. 메뉴 패널 반투명화.
- **유리 머티리얼 분리**: 창문을 별도 광택 재질로 렌더(도색과 분리 베이크).

## v1.5 (2026-07-05)
- **서스펜션 랩** (맵 8): 6개 레인 — 방지턱 4/6/8/10/13/16cm(노면 높이 라벨),
  빨래판 슬랫 26개, 트위스트(좌우 엇갈림 범프), 언덕 1.5/3/5/8m, 경사
  8°/15°/25°(오르막-정상-내리막), 자갈밭·3단 계단·연석 타격 구간.
- **변형 강화**: 소프트바디 충격 반경/깊이 +40%, 항복 변형률 4.5→3.2%,
  프레임 앵커 완화, 임팩트 등록 문턱 2.0→1.4m/s — 가벼운 접촉도 흔적이 남고
  대형 충돌은 훨씬 깊게 접힘 (120km/h 변형지수 22→45).
- **세로모드 차량 썸네일 수정**: 별도 GL 컨텍스트 → 메인 렌더러 렌더타겟
  (readRenderTargetPixels + 감마 보정) — 모바일 세로에서도 3D 프리뷰 표시.
- **UI**: 카드 스태거 등장 애니메이션, 모드 아이콘 그라데이션 타일, CTA 버튼
  시인(광택 스윕), 메탈릭 페인트 하이라이트.
